...
export default async function FechamentoDetailPage({
  ...
  const f = await prisma.espelhoFechamento.findUnique({
    ...
    include: {
      ...
      ocorrencias: {
        orderBy: { data: "asc" },
        include: { correcao: { select: { codigo: true } } },
      },
  ...
  const canManageDocuments = podeEditar(user, "PONTO")
  ...
  const ocorrencias: OcorrenciaView[] = f.ocorrencias.map((o) => ({
    ...
    correcaoCodigo: o.correcao?.codigo ?? null,
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      ...
      >
        <ButtonLink
          variant="outline"
          href={`/rh/ponto?comp=${comp || f.competencia}`}
        >
